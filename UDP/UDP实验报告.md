...
## 有待改进
时间精力所限，感觉有很多地方可以改进，留待之后再做添改

1. 协议效率
	
	数据包的传输使用串行模式，即上一个数据包确认后再发送下一个。这种模式比较安全，且实现简单，但显然效率低下。分组数据包并行发送和确认能够大大提高效率。
	
2. 程序鲁棒性

	在原本的设计中，有一套错误码检查机制（客户端源代码中还保留了rtn_code.h），以对每一个函数进行返回值检查，使得程序出错时可以方便地定位和纠错，这对于一套鲁棒的系统而言非常重要，可惜时间所限，最终版本未能实现完整的返回值检查，今后有机会会添加。

3. 多客户端同时请求

	由于使用了上一节描述的END OF SESSION处理方法，这套协议在同时接受多客户端请求时存在缺陷，因为服务器会将时间戳不属于当前会话的数据包作为当前会话结束的标志。因为刚开始时没有考虑多客户端的情况，否则应当从整体设计上进行改进……

3. 更多特殊情况
	
	比如老师课上提到的由于系统分时造成的各种问题等，这些特殊情况的考虑和处理还需要更多知识和经验的积累。
